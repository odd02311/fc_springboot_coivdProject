...
class ErrorCodeTest {
    ...
    void givenExceptionWithMessage_whenGettingMessage_thenReturnsMessage(ErrorCode sut, String expected) {
        // Given
        Exception e = new Exception("This is test error message");
    ...
    static Stream<Arguments> givenExceptionWithMessage_whenGettingMessage_thenReturnsMessage(){
        return Stream.of(
            arguments(ErrorCode.OK, "Ok - This is test error message"),
            arguments(ErrorCode.BAD_REQUEST, "Bad request - This is test error message"),
            arguments(ErrorCode.SPRING_BAD_REQUEST, "Spring-detected bad request - This is test error message"),
            arguments(ErrorCode.VALIDATION_ERROR, "Validation error - This is test error message"),
            arguments(ErrorCode.INTERNAL_ERROR, "Internal error - This is test error message"),
            arguments(ErrorCode.SPRING_INTERNAL_ERROR, "Spring-detected internal error - This is test error message"),
            arguments(ErrorCode.DATA_ACCESS_ERROR, "Data access error - This is test error message")
        );
    }

    @ParameterizedTest(name = "[{index}] \"{0}\" ====> \"{1}\"")
    @MethodSource
    @DisplayName("메세지를 받으면, 해당 에러 메세지 출력")
